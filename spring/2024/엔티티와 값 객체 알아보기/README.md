...
서로 다른 두 객체가 같은 참조를 바라볼 때, 이를 같다고 한다.
...
클래스에 ID와 같은 필드가 있을 때, 서로 다른 두 클래스의 객체는 식별자가 같으면 같다고 간주한다.

[그림2]

### 구조적인 같음(Structural equality)
마지막으로 두 객체의 모든 멤버가 같을 때 구조적으로 같다고 한다. 이를 동등성이라고도 표현한다.
...
예를 들어 A가 소유한 '5천 원'이란 값 객체는 B에게 넘겨도 그 가치는 동일하며 상호 교환이 가능하다. 하지만 엔티티는 사뭇 다르다. 서로 다른 A와 B가 이름도 같고 성별도 같고 심지어 나이까지 같더라도 Id가 다르면 다른 엔티티로 간주한다.

### 여담: Spring Data JPA는 엔티티의 '동일성'을 보장xw해준다.
JPA는 기본적으로 1차 캐시를 통해 엔티티를 관리함.

특히 엔티티를 다룰 때, 처음 로드된 엔티티가 프록시(Proxy)인지 아닌지에 따라 이후 로드된 동일 엔티티의 동작이 달라진다.
1. 초기 로드된 엔티티가 프록시인 경우 이후 동일한 엔티티를 로드할 때도 프록시로 반환한다.
2. 초기 로드된 엔티티가 프록시가 아닌 경우 이후 동일 엔티티도 프록시가 아닌 실제 엔티티로 반환합니다. (fetchJoin 등을 하지 않았어도!)

결과적으로, JPA는 같은 엔티티를 항상 동일한 참조(Reference)를 보장하므로 `== 비교`를 통해도 비교할 수 있다.

반면 값 객체는 구조적으로 모든 멤버가 같아야 같음을 보장한다. 즉 동등성 비교를 통해 같음을 보장한다. 따라서 값 객체를 설계할 때는 롬복의 @EqualsAndHashCode를 사용하거나 직접 Equals와 Hashcode 를 구현하여 동등성을 보장해야 한다 생각함.
